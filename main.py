...
adj = (
    scipy_sparse_mat_to_torch_sparse_tensor(train).coalesce().cuda(torch.device(device))
)
    ...
    if epoch % 3 == 0:  # test every 10 epochs
        ...
        for batch in tqdm(range(batch_no)):
            ...
            recall_20, ndcg_20 = metrics(
                test_uids[start:end], predictions, 20, test_labels
            )
            # top@40
            recall_40, ndcg_40 = metrics(
                test_uids[start:end], predictions, 40, test_labels
            )
        ...
        print(
            ...
            ":",
            "Recall@20:",
            all_recall_20 / batch_no,
            "Ndcg@20:",
            all_ndcg_20 / batch_no,
            "Recall@40:",
            all_recall_40 / batch_no,
            "Ndcg@40:",
            all_ndcg_40 / batch_no,
        )
...
metric.to_csv(
    ...
    + args.data
    + "_"
    + time.strftime("%Y-%m-%d_%H_%M_%S", current_t)
    + ".csv"
)

torch.save(
    model.state_dict(),
    "saved_model/saved_model_"
    + args.data
    + "_"
    + time.strftime("%Y-%m-%d_%H_%M_%S", current_t)
    + ".pt",
